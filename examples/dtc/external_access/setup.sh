#!/bin/bash

# SPDX-License-Identifier: Apache-2.0
# Copyright (c) 2021 Intel Corporation

set -o errexit
set -o nounset
set -o pipefail

HOST_IP=${HOST_IP:-"oops"}
KUBE_PATH1=${KUBE_PATH1:-"oops"}
KUBE_PATH2=${KUBE_PATH2:-"oops"}
CLIENT_SERVICE_IMAGE_REPOSITORY=${CLIENT_SERVICE_IMAGE_REPOSITORY:-"<docker-registry-url>/httpbin-client"}
CLUSTER1_ISTIO_INGRESS_GATEWAY_ADDRESS=${CLUSTER1_ISTIO_INGRESS_GATEWAY_ADDRESS:-172.16.16.100}
CLUSTER2_ISTIO_INGRESS_GATEWAY_ADDRESS=${CLUSTER2_ISTIO_INGRESS_GATEWAY_ADDRESS:-172.16.16.200}
# tar files
function create {
    # make the GMS helm charts and profiles
    mkdir -p output
    tar -czf output/httpbin.tgz -C ../../helm_charts/httpbin-server/helm httpbin
    tar -czf output/sleep.tgz -C ../../helm_charts/httpbin-client/helm sleep
    tar -czf output/httpbin-server-profile.tar.gz -C ../../helm_charts/httpbin-server/profile .
    tar -czf output/sleep-client-profile.tar.gz -C ../../helm_charts/httpbin-client/profile .


        cat << NET > values.yaml
    ClusterProvider: provider
    Cluster1: httpbincluster
    Cluster2: sleepcluster
    KubeConfig1: $KUBE_PATH1
    KubeConfig2: $KUBE_PATH2
    ProjectName: httpbinproj
    LogicalCloud1RefName: httpbinlc
    LogicalCloud2RefName: sleeplc
    Cluster1Label: edge-cluster1
    Cluster2Label: edge-cluster2
    Cluster1IstioIngressGatewayKvName: httpbinistioingresskvpairs
    Cluster2IstioIngressGatewayKvName: sleepistioingresskvpairs
    Cluster1IstioIngressGatewayAddress: $CLUSTER1_ISTIO_INGRESS_GATEWAY_ADDRESS
    Cluster2IstioIngressGatewayAddress: $CLUSTER2_ISTIO_INGRESS_GATEWAY_ADDRESS
    AdminCloud: default
    LogicalCloud: std-lc
    LogicalCloudNamespace: httpbin-ns
    IstioNamespace: istio-system
    LogicalCloudPermission: standard-permission
    IstioPermission: istio-permission
    CompositeApp: httpbin-collection-composite-app
    CompositeAppVersion: v1
    Applist:
      - httpbin
      - sleep
    AppsInCluster1:
      - httpbin
    AppsInCluster2:
      - sleep
    CompositeProfile: httpbin-collection-composite-profile
    DeploymentIntentGroup: httpbin-collection-deployment-intent-group
    DeploymentIntent: httpbin-collection-deployment-intent
    GenericPlacementIntent: httpbin-collection-placement-intent
    GenericPlacementIntent2: sleep-collection-placement-intent
    DtcIntent: httpbintestdtc
    DtcHttpbinServerIntent: httpbinserver
    DtcHttpbinServerIntentHTTP: httpbinserverhttp
    Intent: httpbinintent
    DtcClientsIntent: sleepclient
    DtcClientsAccessIntent: allowstatus
    ServiceCertificate: ""
    ServicePrivateKey: ""
    CaCertificate: ""
    RsyncPort: 30431
    DtcPort: 30448
    ItsPort: 30440
    HostIP: $HOST_IP
    ClientServiceImageRepository: $CLIENT_SERVICE_IMAGE_REPOSITORY
NET
cat << NET > emco-cfg.yaml
  orchestrator:
    host: $HOST_IP
    port: 30415
  clm:
    host: $HOST_IP
    port: 30461
  ncm:
    host: $HOST_IP
    port: 30481
  ovnaction:
    host: $HOST_IP
    port: 30451
  dcm:
    host: $HOST_IP
    port: 30477
  gac:
    host: $HOST_IP
    port: 30420
  dtc:
   host: $HOST_IP
   port: 30418
  rsync:
   host: $HOST_IP
   port: 30431
NET

}

function usage {
    echo "Usage: $0  create|cleanup"
}

function cleanup {
    rm -f *.tar.gz
    rm -f values.yaml
    rm -f emco-cfg.yaml
    rm -rf output
}

if [ "$#" -lt 1 ] ; then
    usage
    exit
fi

case "$1" in
    "create" )
        if [ "${HOST_IP}" == "oops" ] || [ "${KUBE_PATH1}" == "oops" ] || [ "${KUBE_PATH2}" == "oops" ]; then
            echo -e "ERROR - HOST_IP, KUBE_PATH1 & KUBE_PATH2 environment variable needs to be set"
        else
            create
        fi
        ;;
    "cleanup" )
        cleanup
    ;;
    *)
        usage ;;
esac
