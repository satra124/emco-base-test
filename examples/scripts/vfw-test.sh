...
lc_cluster_2_name="lc1-c2"
cluster_2_data="$(cat << EOF
{
 "metadata" : {
    "name": "${lc_cluster_2_name}",
    "description": "logical cloud cluster 2 description",
    "userData1":"<user data>",
    "userData2":"<user data>"
   },
...
vfw_compositeapp_data="$(cat <<EOF
{
  "metadata": {
   ...
   "spec":{
      "compositeAppVersion":"${vfw_compositeapp_version}"
   }
}
EOF
)"
...
deployment_intent_group_data="$(cat <<EOF
{
   ...
   "spec":{
      "compositeProfile":"${vfw_composite_profile_name}",
      "version":"${release}",
      "logicalCloud":"${admin_logical_cloud_name}",
      "overrideValues":[
         {
            "app":"${packetgen_app_name}",
            "values": {
...
packetgen_workload_intent_data="$(cat <<EOF
{
  ...
  "spec": {
    "app": "${packetgen_app_name}",
    "workloadResource": "${release}-${packetgen_app_name}",
    "type": "Deployment"
  }
}
EOF
...
firewall_workload_intent_data="$(cat <<EOF
{
  ...
  "spec": {
    "app": "${firewall_app_name}",
    "workloadResource": "${release}-${firewall_app_name}",
    "type": "Deployment"
  }
}
EOF
...
sink_workload_intent_data="$(cat <<EOF
{
  ...
  "spec": {
    "app": "${sink_app_name}",
    "workloadResource": "${release}-${sink_app_name}",
    "type": "Deployment"
  }
}
EOF
...
function createLogicalCloudData {
    ...
    call_api -d "${cluster_2_data}" "${base_url_dcm}/projects/${projectname}/logical-clouds/${admin_logical_cloud_name}/cluster-references"
}
...
function getLogicalCloudData {
    ...
    call_api_nox "${base_url_dcm}/projects/${projectname}/logical-clouds/${admin_logical_cloud_name}/cluster-references/${lc_cluster_2_name}"
}
...
function deleteLogicalCloud {
    ...
    delete_resource "${base_url_dcm}/projects/${projectname}/logical-clouds/${admin_logical_cloud_name}/cluster-references/${lc_cluster_2_name}"
...
# also instantiate the logical cloud
