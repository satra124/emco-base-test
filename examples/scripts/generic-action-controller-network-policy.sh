...
compositeapp_data="$(cat <<EOF
{
  "metadata": {
   ...
   "spec":{
      "compositeAppVersion":"${compositeapp_version}"
   }
}
EOF
)"
...
deployment_intent_group_data="$(cat <<EOF
{
   ...
   "spec":{
      "compositeProfile":"${operators_composite_profile_name}",
      "version":"${release}",
...
# define the resource
resource_name="resourceNetworkPolicyApp"
...
resource_data_file="/opt/kud/multi-cluster/cluster1/artifacts/networkPolicy.yaml"


# define the customization
customization_name="customizeNetworkPolicy"
...
function createGenericActionData {
   ...
   call_api -H "Content-Type: multipart/form-data" -F "metadata=$resource_data" -F "file=@$resource_data_file" "${base_url_gac}/projects/${projectname}/composite-apps/${operators_compositeapp_name}/${compositeapp_version}/deployment-intent-groups/${deployment_intent_group_name}/generic-k8s-intents/${generick8s_intent_name}/resources"

   print_msg "BEGIN :: create customization_data"
