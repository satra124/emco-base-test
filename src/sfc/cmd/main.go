...
import (
	...
	register "gitlab.com/project-emco/core/emco-base/src/sfc/pkg/grpc"
...
func startGrpcServer() error {
	...
	log.Printf("Starting SFC Action Controller gRPC Server on port %v", port)
	err = grpcServer.Serve(lis)
	if err != nil {
		log.Fatalf("sfc grpc server is not serving %v", err)
	}
	return err
}
...
func main() {
	...
	log.Printf("Starting SFC Action  Controller on port %v", config.GetConfiguration().ServicePort)
