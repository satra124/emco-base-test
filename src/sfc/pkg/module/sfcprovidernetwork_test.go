...
var _ = Describe("SFCProviderNetworkIntent", func() {
	...
	Describe("Create SFC provider network intent", func() {
		It("successful creation of sfc provider network intent", func() {
			...
			_, err = (*sfcClient).CreateSfcProviderNetworkIntent(sfcProviderNetworkIntent, "testproject", "ca", "v1", "dig", "sfcIntentName", false)
			Expect(err).To(BeNil())
		})
		...
		It("successful creation of sfc provider network intent with update version of call", func() {
			...
			_, err = (*sfcClient).CreateSfcProviderNetworkIntent(sfcProviderNetworkIntent, "testproject", "ca", "v1", "dig", "sfcIntentName", true)
			Expect(err).To(BeNil())
		})
		It("successful creation of sfc provider network intent with update version of call", func() {
			// set up prerequisites
	...
	Describe("Get all sfc provider network intents", func() {
		It("Parent SFC Intent does not exist - return not found error", func() {
			_, err := (*sfcClient).GetAllSfcProviderNetworkIntents("testproject", "ca", "v1", "dig", "sfcIntentName")
		...
		It("Parent SFC Intent does exist - No SFC provider network Intents - should return empty list", func() {
			...
			list, err := (*sfcClient).GetAllSfcProviderNetworkIntents("testproject", "ca", "v1", "dig", "sfcIntentName")
	...
	Describe("Get sfc provider network intent", func() {
		It("Parent SFC Intent does not exist - return not found error", func() {
			_, err := (*sfcClient).GetSfcProviderNetworkIntent("sfcIntentName", "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(strings.Contains(err.Error(), "not found")).To(Equal(true))
		})
		It("Successful get of sfcProviderNetworkIntent", func() {
			...
			_, err = (*sfcClient).GetSfcProviderNetworkIntent("sfcProviderNetworkIntentName", "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(err).To(BeNil())
		})
		It("should return error for general db error", func() {
			mdb.Err = pkgerrors.New("db Find error")
	...
	Describe("Delete SFC provider network intent", func() {
		It("successful delete", func() {
			...
			err = (*sfcClient).DeleteSfcProviderNetworkIntent("sfcProviderNetworkIntentName", "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(err).To(BeNil())
		})
