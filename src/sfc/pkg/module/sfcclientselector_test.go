...
var _ = Describe("SFCClientSelectorIntent", func() {
	...
	Describe("Create SFC client selector intent", func() {
		It("successful creation of sfc client selector intent", func() {
			...
			_, err = (*sfcClient).CreateSfcClientSelectorIntent(ctx, sfcClientSelectorIntent, "testproject", "ca", "v1", "dig", "sfcIntentName", false)
			Expect(err).To(BeNil())
		})
		...
		It("successful creation of sfc client selector intent with update version of call", func() {
			...
			_, err = (*sfcClient).CreateSfcClientSelectorIntent(ctx, sfcClientSelectorIntent, "testproject", "ca", "v1", "dig", "sfcIntentName", true)
			Expect(err).To(BeNil())
		})
		It("successful creation of sfc client selector intent with update version of call", func() {
			ctx := context.Background()
	...
	Describe("Get all sfc client selector intents", func() {
		It("Parent SFC Intent does not exist - return not found error", func() {
			ctx := context.Background()
			_, err := (*sfcClient).GetAllSfcClientSelectorIntents(ctx, "testproject", "ca", "v1", "dig", "sfcIntentName")
		...
		It("Parent SFC Intent does exist - No SFC Client Selector Intents - should return empty list", func() {
			...
			list, err := (*sfcClient).GetAllSfcClientSelectorIntents(ctx, "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(err).To(BeNil())
	...
	Describe("Get sfc client selector intent", func() {
		It("Parent SFC Intent does not exist - return not found error", func() {
			ctx := context.Background()
			_, err := (*sfcClient).GetSfcClientSelectorIntent(ctx, "sfcIntentName", "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(strings.Contains(err.Error(), "not found")).To(Equal(true))
		})
		It("Successful get of sfcClientSelectorIntent", func() {
			...
			_, err = (*sfcClient).GetSfcClientSelectorIntent(ctx, "sfcClientSelectorIntentName", "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(err).To(BeNil())
		})
		It("should return error for general db error", func() {
			ctx := context.Background()
	...
	Describe("Delete SFC Client Selector intent", func() {
		It("successful delete", func() {
			...
			err = (*sfcClient).DeleteSfcClientSelectorIntent(ctx, "sfcClientSelectorIntentName", "testproject", "ca", "v1", "dig", "sfcIntentName")
			Expect(err).To(BeNil())
		})
