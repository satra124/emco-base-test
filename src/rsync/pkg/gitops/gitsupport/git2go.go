...
func (p *GitProvider) CommitFiles(message string, files []CommitFile) error {
...
func (p *GitProvider) PushBranch(repo *git.Repository) error {
...
func (p *GitProvider) GitPull(folderName, branchName string) error {
	url := p.Url
	userName := p.UserName
	check, err := Exists(folderName)
