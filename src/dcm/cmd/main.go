...
func startGrpcServer() error {
	...
	_, port := register.GetServerHostPort("dcm", "DCM_NAME", 9078)
	...
	log.Println("Starting DCM Status Notification gRPC Server")
	err = grpcServer.Serve(lis)
	if err != nil {
		log.Fatalf("DCM Status Notification gRPC server is not serving %v", err)
	}
	return err
}
