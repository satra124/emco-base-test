...
func TestGenerateKubernetesArtifacts(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			chartPath:  "<path to helm chart>/helm_charts/dtc/http-server",
			valueFiles: []string{},
			values:     []string{},
			//sha256 hash of the evaluated templates in each chart
			expectedHashMap: map[string]string{
		...
		},
		{
			...
			chartPath:  "<path to helm chart>/app2/helm/prometheus-operator",
			valueFiles: []string{},
			values:     []string{},
			//sha256 hash of the evaluated templates in each chart
			expectedHashMap: map[string]string{
	...
	for _, testCase := range testCases {
		// Disabling Prometheus and http-server tests
		// TODO: Copy the Helm charts to mock and enable the tests
		if strings.Contains(testCase.label, "prometheus") || strings.Contains(testCase.label, "http-server"){
			t.Skip()
		}
		t.Run(testCase.label, func(t *testing.T) {
			tc := NewTemplateClient("1.12.3", "testnamespace", "testreleasename", "manifest.yaml")
