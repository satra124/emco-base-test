...
class TestCase_getControllers:
    def test_getControllers(self, cwd, orchestrator_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    ...
                    self.get_controllers_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_controllers(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_controllers_assertion_check(
