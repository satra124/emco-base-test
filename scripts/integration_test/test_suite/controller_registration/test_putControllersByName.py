...
class TestCase_putControllersByName:
    def test_putControllersByName(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.put_controllers(
                        orchestrator_url,
                        params_from_inputfile.get_anchor(0),
                        params_from_inputfile.get_request_body_controller(0, item),
                    )
                    logfile.log_debug(
                    ...
                    self.put_controllers_assertion_check(
                        logfile,
                        response,
                        params_from_inputfile.get_response_code(0, item),
                        params_from_inputfile.get_response_text_controller(0, item),
                    )
    ...
    @classmethod
    def put_controllers(self, orchestrator_url, anchor, request_body=None):
    ...
    @classmethod
    def put_controllers_assertion_check(
