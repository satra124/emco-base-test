...
class TestCase_getControllersByName:
    def test_getControllersByName(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_controller_by_name(
                    ...
                    self.get_controller_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_controller_by_name(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_controller_by_name_assertion_check(
