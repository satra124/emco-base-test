...
class TestCase_terminateDeployment:
    def test_terminateDeployment(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.post_terminateDeployment(
                    ...
                    self.post_terminate_deployment_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def post_terminateDeployment(self, orchestrator_url, anchor):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        response = requests.post(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def post_terminate_deployment_assertion_check(
