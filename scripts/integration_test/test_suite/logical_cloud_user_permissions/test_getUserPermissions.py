...
class TestCase_getUserPermissions:
    def test_getUserPermissions(self, cwd, dcm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_user_permissions(
                    ...
                    self.get_user_permissions_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_user_permissions(self, dcm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dcm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_user_permissions_assertion_check(
