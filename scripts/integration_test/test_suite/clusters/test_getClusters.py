...
class TestCase_getclusters:
    def test_getclusters(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    ...
                    self.get_clusters_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_clusters(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_clusters_assertion_check(
