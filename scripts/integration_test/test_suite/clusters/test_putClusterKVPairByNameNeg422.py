...
class TestCase_putClusterKVPairs:
    def test_putClusterKVPairsByName(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.put_clustersKVPairs(
                        clm_url,
                    ...
                    self.put_clustersKVPairs_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def put_clustersKVPairs(self, clm_url, anchor, requestbody=None):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = requestbody
        response = requests.put(clm_url + anchor, data=payload, headers=headers)
        return response

    @classmethod
    def put_clustersKVPairs_assertion_check(
