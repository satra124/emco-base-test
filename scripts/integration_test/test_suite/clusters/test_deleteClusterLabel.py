...
class TestCase_delClusterLabel:
    def test_delClusterLabel(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.delete_cluster_label_by_name(
                        clm_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    self.delete_cluster_label_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def delete_cluster_label_by_name(self, clm_url, anchor):
        headers = {"accept": "*/* "}
        response = requests.delete(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_cluster_label_by_name_assertion_check(
