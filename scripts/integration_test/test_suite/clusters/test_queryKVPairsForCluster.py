...
class TestCase_getKVPairsForCluster:
    def test_getKVPairsForCluster(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_KVPairs_for_cluster(
                    ...
                    self.get_KVPairs_for_cluster_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_KVPairs_for_cluster(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_KVPairs_for_cluster_assertion_check(
