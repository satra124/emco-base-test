...
class TestCase_getclusterKVPairs:
    def test_getclusterKVPairs(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_clusterKVPairs(
                    ...
                    self.get_cluster_kvpairs_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_clusterKVPairs(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_cluster_kvpairs_assertion_check(
