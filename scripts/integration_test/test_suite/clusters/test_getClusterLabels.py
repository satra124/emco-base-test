...
class TestCase_getclusterLabels:
    def test_getclusterLabels(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_cluster_labels(
                    ...
                    self.get_cluster_labels_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_cluster_labels(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_cluster_labels_assertion_check(
