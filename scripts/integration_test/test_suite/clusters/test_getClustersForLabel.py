...
class TestCase_getclustersForLabel:
    def test_getclustersForLabel(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_clusters_for_label(
                    ...
                    self.get_clusters_for_label_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_clusters_for_label(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_clusters_for_label_assertion_check(
