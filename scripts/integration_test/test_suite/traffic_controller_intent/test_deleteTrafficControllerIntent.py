...
class TestCase_deleteTrafficControllerIntent:
    def test_deleteTrafficControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.delete_traffic_controller_intent(
                        dtc_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    self.delete_traffic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def delete_traffic_controller_intent(self, dtc_url, anchor):
        headers = {"accept": "*/*"}
        response = requests.delete(dtc_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_traffic_controller_intent_assertion_check(
