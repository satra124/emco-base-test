...
class TestCase_postServerInboundIntentTrafficControllerIntent:
    def test_postServerInboundIntentTrafficControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.post_server_inbound_intent_traffic_controller_intent(
                        dtc_url,
                    ...
                    self.post_server_inbound_intent_traffic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def post_server_inbound_intent_traffic_controller_intent(
        self, dtc_url, anchor, requestbody=None
    ):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = requestbody
        response = requests.post(dtc_url + anchor, data=payload, headers=headers)
        return response

    @classmethod
    def post_server_inbound_intent_traffic_controller_intent_assertion_check(
