...
class TestCase_getServerInboundIntentTrafficControllerIntentByName:
    def test_getServerInboundIntentTrafficControllerIntentByName(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_server_inbound_intent_traffic_controller_intent_by_name(
                        dtc_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    logfile.log_debug("Received Response Text: " + str(response.text))
                    self.get_server_inbound_intent_traffic_controller_intent_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_server_inbound_intent_traffic_controller_intent_by_name(
        self, dtc_url, anchor
    ):
        headers = {"accept": "application/json"}
        response = requests.get(dtc_url + anchor, headers=headers)
        return response

    @classmethod
    def get_server_inbound_intent_traffic_controller_intent_by_name_assertion_check(
