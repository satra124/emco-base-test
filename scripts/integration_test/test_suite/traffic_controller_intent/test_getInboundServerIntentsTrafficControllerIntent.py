...
class TestCase_getServerInboundIntentsTrafficControllerIntent:
    def test_getServerInboundIntentsTrafficControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_server_inbound_intent_traffic_controller_intent(
                    ...
                    self.get_server_inbound_intent_traffic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_server_inbound_intent_traffic_controller_intent(self, dtc_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dtc_url + anchor, headers=headers)
        return response

    @classmethod
    def get_server_inbound_intent_traffic_controller_intent_assertion_check(
