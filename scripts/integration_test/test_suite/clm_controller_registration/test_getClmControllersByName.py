...
class TestCase_getClmControllersByName:
    def test_getClmControllersByName(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_clm_controller_by_name(
                        clm_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    logfile.log_debug("Received Response Text: " + str(response.text))
                    self.get_clm_controller_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_clm_controller_by_name(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_clm_controller_by_name_assertion_check(
