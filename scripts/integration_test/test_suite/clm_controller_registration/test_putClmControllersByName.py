...
class TestCase_putClmControllersByName:
    def test_putClmControllersByName(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.put_clm_controllers(
                        clm_url,
                    ...
                    self.put_clm_controllers_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def put_clm_controllers(self, clm_url, anchor, request_body=None):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = request_body
        response = requests.put(
            clm_url + anchor, data=json.dumps(payload), headers=headers
        )
        return response

    @classmethod
    def put_clm_controllers_assertion_check(
