...
class TestCase_getClmControllers:
    def test_getClmControllers(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_clm_controllers(
                    ...
                    self.get_clm_controllers_assertion_check(
                        logfile,
                        response,
                        params_from_inputfile.get_response_code(0, item),
                        params_from_inputfile.get_response_text_controller_getAll(
                            0, item
                        ),
                    )
    ...
    @classmethod
    def get_clm_controllers(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_clm_controllers_assertion_check(
