...
class TestCase_deleteHPAResourceConsumerHPAIntent:
    def test_deleteHPAResourceConsumerHPAIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.delete_hpa_resource_consumer_hpa_intent(
                        hpa_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    self.delete_hpa_resource_consumer_hpa_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def delete_hpa_resource_consumer_hpa_intent(self, hpa_url, anchor):
        headers = {"accept": "*/*"}
        response = requests.delete(hpa_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_hpa_resource_consumer_hpa_intent_assertion_check(
