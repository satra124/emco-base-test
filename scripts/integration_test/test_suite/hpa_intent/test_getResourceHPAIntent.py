...
class TestCase_getResourceHPAIntent:
    def test_getResourceHPAIntent(self, cwd, hpa_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_resource_hpa_intent(
                    ...
                    self.get_resource_hpa_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_resource_hpa_intent(self, hpa_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(hpa_url + anchor, headers=headers)
        return response

    @classmethod
    def get_resource_hpa_intent_assertion_check(
