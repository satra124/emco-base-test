...
class TestCase_getResourceHPAIntentByName:
    def test_getResourceHPAIntentByName(self, cwd, hpa_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_resource_hpa_intent_by_name(
                    ...
                    self.get_resource_hpa_intent_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_resource_hpa_intent_by_name(self, hpa_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(hpa_url + anchor, headers=headers)
        return response

    @classmethod
    def get_resource_hpa_intent_by_name_assertion_check(
