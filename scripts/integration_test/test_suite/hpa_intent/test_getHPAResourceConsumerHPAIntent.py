...
class TestCase_getHPAResourceConsumerHPAIntent:
    def test_getHPAResourceConsumerHPAIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_hpa_resource_consumer_hpa_intent(
                    ...
                    self.get_hpa_resource_consumer_hpa_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_hpa_resource_consumer_hpa_intent(self, hpa_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(hpa_url + anchor, headers=headers)
        return response

    @classmethod
    def get_hpa_resource_consumer_hpa_intent_assertion_check(
