...
class TestCase_getHPAIntentByName:
    def test_getHPAIntentByName(self, cwd, hpa_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_hpa_intent_by_name(
                    ...
                    self.get_hpa_intent_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_hpa_intent_by_name(self, hpa_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(hpa_url + anchor, headers=headers)
        return response

    @classmethod
    def get_hpa_intent_by_name_assertion_check(
