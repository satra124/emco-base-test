...
class TestCase_queryHPAIntent:
    def test_queryHPAIntent(self, cwd, hpa_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.query_hpa_intent(
                    ...
                    self.query_hpa_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def query_hpa_intent(self, hpa_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(hpa_url + anchor, headers=headers)
        return response

    @classmethod
    def query_hpa_intent_assertion_check(
