...
class TestCase_getIntentForApp:
    def test_getIntentForApp(self, cwd, orchestrator_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_intent_for_app(
                    ...
                    self.get_intent_for_app_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_intent_for_app(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_intent_for_app_assertion_check(
