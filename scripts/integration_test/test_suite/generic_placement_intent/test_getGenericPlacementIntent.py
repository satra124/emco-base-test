...
class TestCase_getGenericPlacementIntent:
    def test_getGenericPlacementIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_generic_placement_intent(
                    ...
                    self.get_generic_placement_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_generic_placement_intent(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_generic_placement_intent_assertion_check(
