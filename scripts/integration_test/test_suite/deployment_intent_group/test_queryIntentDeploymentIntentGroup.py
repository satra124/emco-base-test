...
class TestCase_queryIntentDeploymentIntentGroup:
    def test_queryIntentDeploymentIntentGroup(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.query_intent_deployment_intent_group(
                    ...
                    self.query_intent_deployment_intent_group_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def query_intent_deployment_intent_group(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def query_intent_deployment_intent_group_assertion_check(
