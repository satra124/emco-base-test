...
class TestCase_getDeploymentIntentGroup:
    def test_getDeploymentIntentGroup(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_deployment_intent_group(
                    ...
                    self.get_deployment_intent_group_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_deployment_intent_group(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_deployment_intent_group_assertion_check(
