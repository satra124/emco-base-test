...
class TestCase_deleteIntentDeploymentIntentGroup:
    def test_deleteIntentDeploymentIntentGroup(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.delete_intent_deployment_intent_group(
                        orchestrator_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    self.delete_intent_deployment_intent_group_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def delete_intent_deployment_intent_group(self, orchestrator_url, anchor):
        headers = {"accept": "*/*"}
        response = requests.delete(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_intent_deployment_intent_group_assertion_check(
