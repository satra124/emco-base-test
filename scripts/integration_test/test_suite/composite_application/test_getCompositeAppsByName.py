...
class TestCase_getcompositeAppsByName:
    def testCase_getcompositeAppsByName(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_composite_apps_by_name(
                    ...
                    self.get_composite_apps_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_composite_apps_by_name(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_composite_apps_by_name_assertion_check(
