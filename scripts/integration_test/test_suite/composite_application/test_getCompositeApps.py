...
class TestCase_getcompositeApps:
    def testCase_getcompositeApps(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_composite_apps(
                    ...
                    self.get_composite_apps_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_composite_apps(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_composite_apps_assertion_check(
