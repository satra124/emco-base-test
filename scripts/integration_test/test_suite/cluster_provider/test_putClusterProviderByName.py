...
class TestCase_putClusterProviderByName:
    def test_putClusterProviderByName(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.put_clusterprovider_by_name(
                        clm_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    self.put_clusterprovider_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def put_clusterprovider_by_name(self, clm_url, anchor):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        response = requests.put(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def put_clusterprovider_by_name_assertion_check(
