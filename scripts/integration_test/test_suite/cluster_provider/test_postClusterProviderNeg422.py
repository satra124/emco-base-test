...
class TestCase_clusterProvider:
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.post_clusterprovider(
                        clm_url,
                    ...
                    self.post_clusterprovider_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def post_clusterprovider(self, clm_url, anchor, request_body=None):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = request_body
        response = requests.post(clm_url + anchor, data=payload, headers=headers)
        return response

    @classmethod
    def post_clusterprovider_assertion_check(
