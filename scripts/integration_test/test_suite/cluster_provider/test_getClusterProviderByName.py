...
class TestCase_clusterProviderByName:
    def test_clusterProviderByName(self, cwd, clm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_clusterprovider_by_name(
                    ...
                    self.get_clusterprovider_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_clusterprovider_by_name(self, clm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(clm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_clusterprovider_by_name_assertion_check(
