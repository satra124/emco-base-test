...
class TestCase_getKVPairs:
    def test_getKVPairs(self, cwd, dcm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    ...
                    self.get_kv_pairs_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_kv_pairs(self, dcm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dcm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_kv_pairs_assertion_check(
