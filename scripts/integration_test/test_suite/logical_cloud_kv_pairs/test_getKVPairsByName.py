...
class TestCase_getKVPairsByName:
    def test_getKVPairsByName(self, cwd, dcm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_kv_pair_by_name(
                    ...
                    self.get_kv_pair_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_kv_pair_by_name(self, dcm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dcm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_kv_pair_by_name_assertion_check(
