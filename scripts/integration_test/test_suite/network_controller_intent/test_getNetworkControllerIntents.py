...
class TestCase_getNetworkControllerIntents:
    def test_getNetworkControllerIntents(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_network_controller_intent(
                    ...
                    self.get_network_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_network_controller_intent(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_network_controller_intent_assertion_check(
