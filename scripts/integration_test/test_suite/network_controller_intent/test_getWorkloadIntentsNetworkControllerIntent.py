...
class TestCase_getWorkloadIntentsNetworkControllerIntent:
    def test_getWorkloadIntentsNetworkControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_workload_intent_network_controller_intent(
                    ...
                    self.get_workload_intent_network_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_workload_intent_network_controller_intent(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_workload_intent_network_controller_intent_assertion_check(
