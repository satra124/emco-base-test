...
class TestCase_postWorkloadInterfaceNetworkControllerIntent:
    def test_postWorkloadInterfaceNetworkControllerIntent(
        self, cwd, orchestrator_url, emcotestlogging, api_input
    ):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.post_workload_interface_network_controller_intent(
                        orchestrator_url,
                    ...
                    self.post_workload_interface_network_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def post_workload_interface_network_controller_intent(
        self, orchestrator_url, anchor, requestbody=None
    ):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = requestbody
        response = requests.post(
            orchestrator_url + anchor, data=json.dumps(payload), headers=headers
        )
        return response

    @classmethod
    def post_workload_interface_network_controller_intent_assertion_check(
