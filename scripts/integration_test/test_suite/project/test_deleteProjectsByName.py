...
class TestCase_delProj:
    def test_deleteProjectsByName(self, cwd, orchestrator_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.delete_projects_by_name(
                    ...
                    self.delete_projects_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def delete_projects_by_name(self, orchestrator_url, anchor):
        headers = {"accept": "*/* "}
        response = requests.delete(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_projects_by_name_assertion_check(
