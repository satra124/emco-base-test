...
class TestCase_getProjByName:
    def test_getProjectsByName(self, cwd, orchestrator_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_projects_by_name(
                    ...
                    self.get_projects_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_projects_by_name(self, orchestrator_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(orchestrator_url + anchor, headers=headers)
        return response

    @classmethod
    def get_projects_by_name_assertion_check(
