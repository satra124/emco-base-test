...
class TestCase_getLogicalClouds:
    def test_getLogicalClouds(self, cwd, dcm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_logical_clouds(
                    ...
                    self.get_logical_clouds_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_logical_clouds(self, dcm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dcm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_logical_clouds_assertion_check(
