...
class TestCase_getProviderNetworkByName:
    def test_getProviderNetworkByName(self, cwd, ncm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_provider_network_by_name(
                    ...
                    self.get_provider_network_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_provider_network_by_name(self, ncm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(ncm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_provider_network_by_name_assertion_check(
