...
class TestCase_getNetworks:
    def test_getNetworks(self, cwd, ncm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    ...
                    self.get_networks_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_networks(self, ncm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(ncm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_networks_assertion_check(
