...
class TestCase_delProviderNetwork:
    def test_delProviderNetwork(self, cwd, ncm_url, emcotestlogging, api_input):
        try:
    ...
    @classmethod
    def delete_provider_network(self, ncm_url, anchor):
        headers = {"accept": "*/* "}
        response = requests.delete(ncm_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_provider_network_assertion_check(
