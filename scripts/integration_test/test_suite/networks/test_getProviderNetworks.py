...
class TestCase_getProviderNetworks:
    def test_getProviderNetworks(self, cwd, ncm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_provider_networks(
                    ...
                    self.get_provider_networks_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_provider_networks(self, ncm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(ncm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_provider_networks_assertion_check(
