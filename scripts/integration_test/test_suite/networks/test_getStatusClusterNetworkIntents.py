...
class TestCase_getStatusClusterNetworkIntents:
    def test_getStatusClusterNetworkIntents(
        self, cwd, ncm_url, emcotestlogging, api_input
    ):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_status_cluster_network_intent(
                    ...
                    self.get_status_cluster_network_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_status_cluster_network_intent(self, ncm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(ncm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_status_cluster_network_intent_assertion_check(
