...
class TestCase_getGenericControllerIntents:
    def test_getGenericControllerIntents(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_generic_controller_intent(
                    ...
                    self.get_generic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_generic_controller_intent(self, gac_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(gac_url + anchor, headers=headers)
        return response

    @classmethod
    def get_generic_controller_intent_assertion_check(
