...
class TestCase_getResourceIntentsGenericControllerIntent:
    def test_getResourceIntentsGenericControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_resource_intent_generic_controller_intent(
                    ...
                    self.get_resource_intent_generic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_resource_intent_generic_controller_intent(self, gac_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(gac_url + anchor, headers=headers)
        return response

    @classmethod
    def get_resource_intent_generic_controller_intent_assertion_check(
