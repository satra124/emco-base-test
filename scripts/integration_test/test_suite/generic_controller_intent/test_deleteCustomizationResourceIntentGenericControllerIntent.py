...
class TestCase_deleteCustomizationResourceIntentGenericControllerIntent:
    def test_deleteCustomizationResourceIntentGenericControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.delete_customization_resource_intent_generic_controller_intent(
                        gac_url, params_from_inputfile.get_anchor(0)
                    )
                    logfile.log_debug(
                        "Received Response Code: " + str(response.status_code)
                    )
                    self.delete_customization_resource_intent_generic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def delete_customization_resource_intent_generic_controller_intent(
        self, gac_url, anchor
    ):
        headers = {"accept": "*/*"}
        response = requests.delete(gac_url + anchor, headers=headers)
        return response

    @classmethod
    def delete_customization_resource_intent_generic_controller_intent_assertion_check(
