...
class TestCase_postGenericControllerIntent:
    def test_postGenericControllerIntent(
        ...
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.post_generic_controller_intent(
                        gac_url,
                    ...
                    self.post_generic_controller_intent_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def post_generic_controller_intent(self, gac_url, anchor, requestbody=None):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = requestbody
        response = requests.post(
            gac_url + anchor, data=json.dumps(payload), headers=headers
        )
        return response

    @classmethod
    def post_generic_controller_intent_assertion_check(
