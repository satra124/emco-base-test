...
class TestCase_getClusterReferenceByName:
    def test_getClusterReferenceByName(self, cwd, dcm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_cluster_reference_by_name(
                    ...
                    self.get_cluster_reference_by_name_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_cluster_reference_by_name(self, dcm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dcm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_cluster_reference_by_name_assertion_check(
