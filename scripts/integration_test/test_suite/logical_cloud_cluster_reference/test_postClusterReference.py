...
class TestCase_postClusterReference:
    def test_postClusterReference(self, cwd, dcm_url, emcotestlogging, api_input):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.post_cluster_reference(
                        dcm_url,
                    ...
                    self.post_cluster_reference_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def post_cluster_reference(self, dcm_url, anchor, requestbody=None):
        headers = {"Content-Type": "application/json", "accept": "application/json"}
        payload = requestbody
        response = requests.post(
            dcm_url + anchor, data=json.dumps(payload), headers=headers
        )
        return response

    @classmethod
    def post_cluster_reference_assertion_check(
