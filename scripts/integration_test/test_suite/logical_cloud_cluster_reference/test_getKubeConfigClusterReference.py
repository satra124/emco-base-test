...
class TestCase_getKubeConfigClusterReference:
    def test_getKubeConfigClusterReference(
        self, cwd, dcm_url, emcotestlogging, api_input
    ):
        try:
            ...
            try:
                for item in range(0, no_of_api_calls):
                    response = self.get_kubeconfig_cluster_reference(
                    ...
                    self.get_kubeconfig_cluster_reference_assertion_check(
                        logfile,
                        response,
    ...
    @classmethod
    def get_kubeconfig_cluster_reference(self, dcm_url, anchor):
        headers = {"accept": "application/json"}
        response = requests.get(dcm_url + anchor, headers=headers)
        return response

    @classmethod
    def get_kubeconfig_cluster_reference_assertion_check(
